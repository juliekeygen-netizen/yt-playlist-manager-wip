...
import type { HistoryRowsPerPage, HistorySavedVideo } from '@shared/historyMockData';
import { DropdownButton } from '../playlists/DropdownButton';
import { StatusText } from '../playlists/StatusBadge';
import { ThumbnailPlaceholder } from '../playlists/ThumbnailPlaceholder';

export interface HistorySelectionModifiers {
  shiftKey: boolean;
  ctrlKey: boolean;
  metaKey: boolean;
...
const tableGrid = 'grid-cols-[42px_88px_minmax(250px,2fr)_minmax(150px,1fr)_96px_100px_36px]';
...
const rowsPerPageOptions = [
  ...
  { label: '50', value: 50 },
  { label: 'All', value: 'All' },
] satisfies Array<{ label: string; value: HistoryRowsPerPage }>;

export function HistorySavedItemsTable({
  videos,
  allVideos,
  selectedIds,
  rowsPerPage,
  ...
  totalPages: number;
  onActivate: () => void;
  onSelectVideo: (id: string, modifiers: HistorySelectionModifiers, visibleIds: string[]) => void;
  onToggleAll: () => void;
  ...
  return (
    <section
      ...
      data-active-list-scope="historySavedVideos"
    ...
    >
      <div className={`grid shrink-0 ${tableGrid} items-center border-y border-white/[0.055] px-4 py-3 text-sm font-medium text-mist-300`}>
        <input
          aria-label="Select saved videos"
          checked={allSelected}
      ...
      <div className="video-table-scroll min-h-0 flex-1 overflow-y-auto">
        {videos.length > 0 ? (
          ...
          <div className="flex h-full min-h-[220px] flex-col items-center justify-center px-6 text-center">
            <h3 className="text-lg font-semibold text-mist-50">No saved videos</h3>
            <p className="mt-2 text-sm text-mist-400">This history item has no saved items.</p>
          </div>
        )}
      </div>
...
function getSelectionModifiers(event: MouseEvent<HTMLElement>): HistorySelectionModifiers {
  return {
